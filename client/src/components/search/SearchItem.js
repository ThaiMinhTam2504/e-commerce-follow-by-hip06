...
const SearchItem = ({ name, activeClick, changeActiveFilter, type = 'checkbox' }) => {
    ...
    useEffect(() => {
        if (price.from && price.to && price.from > price.to) alert('From price must be less than to price')
        if (price.from < 0 || price.to < 0) alert('Price cannot be negative')
    }, [price])
    ...
    return (
        <div
            ...
            className='p-3 cursor-pointer text-gray-500 text-xs relative gap-6 border border-gray-800 flex justify-between items-center'
        >
                        ...
                        {colors.map((el, index) => (
                            <div key={index} className='flex items-center gap-4'>
                                <input
                                    ...
                                    id={el}
                                    checked={selected.some(selectedColor => selectedColor === el)}
                                />
                    ...
                    <div className='flex items-center p-2 gap-2'>
                        ...
                        <div className='flex items-center gap-2'>
                            ...
                            <input
                                ...
                                id='to'
                                value={price.to}
                                onChange={e => setPrice(prev => ({ ...prev, to: e.target.value }))} //return ra object phải bọc trong {}
                            />
                        </div>
                    </div>
                </div>}
